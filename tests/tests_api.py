...
def test_search_in_cyrillic():
    ...
    assert response.status_code == 200, f"Ошибка: ожидался статус код 200, но получен {response.status_code}"
    data = response.json()
...
def test_search_wrong_method():
    ...
    assert response.status_code == 405, f"Ожидался код 405, но получен {response.status_code}"
